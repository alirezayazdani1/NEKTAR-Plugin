...
main (int argc, char *argv[])
{
  ...
  for (j = 0; j < Nsnapshots; ++j){
     ...
     for(i = 0; i < nfields; ++i)
      ...
      Calc_Vort (&f, master,nfields, 0);

     if(option("FEstorage")){
        sprintf(file_name,"VORT_DATA_%d/%s_%d.dat",fieldID[j],file_name_out,mynode());
...
static void Calc_Vort (FileList *f, Element_List **U, int nfields, int Snapshot_index){
  ...
  static  Element_List *T;
  static  Element_List *Tf;
  static  Bsystem *Bsys;

  if(option("PROJECT")){
    if (FLAG_INIT == 0){
      T  = U[0]->gen_aux_field('T');
      Tf = T->gen_aux_field('T');
  ...
  if(U[0]->fhead->dim() == 3){
    ...
    for(k = 0; k < U[0]->nel; ++k){
      ...
      dvsub(qt, Wy, 1, Vz, 1, **U[0]->flist[k]->h_3d, 1);
      dvsub(qt, Uz, 1, Wx, 1, **U[1]->flist[k]->h_3d, 1);
      dvsub(qt, Vx, 1, Uy, 1, **U[2]->flist[k]->h_3d, 1);

      dvadd(qt, Ux, 1, Vy, 1, **U[3]->flist[k]->h_3d, 1);
      dvadd(qt, Wz, 1, **U[3]->flist[k]->h_3d, 1,
                       **U[3]->flist[k]->h_3d, 1);
    }
    ...
    if(option("PROJECT")){
      ...
      dcopy(U[0]->htot, U[0]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      ROOTONLY fprintf(stderr,"Projecting Wx\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[0], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[0]->base_hj, 1);
      U[0]->fhead->type = 'a';


      /* project Wy */

      dcopy(U[1]->htot, U[1]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      ROOTONLY fprintf(stderr,"Projecting Wy\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[1], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[1]->base_hj, 1);
      U[1]->fhead->type = 'b';


     /* project Wz */

      dcopy(U[2]->htot, U[2]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      ROOTONLY fprintf(stderr,"Projecting Wz\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[2], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[2]->base_hj, 1);
      U[2]->fhead->type = 'c';
