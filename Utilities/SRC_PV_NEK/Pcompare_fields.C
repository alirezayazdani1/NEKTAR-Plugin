...
static void WriteTEC(Element_List **E, char *file_name, int nfields, int FLAG_DUMP_INDEX, int Snapshot_index);
...
int read_POD_modes_coef(FILE *pFile, int *index_file);
...
void compare_fields(Element_List **E, int field_index,int field_index2 );
...
main (int argc, char *argv[])
{
  ...
  master  = (Element_List **) malloc((Nsnapshots+1)*4*sizeof(Element_List *));
  nfields = setup (&f, master, &nftot, Nsnapshots);

  ReadCopyField(&f,master);

  for (i = 1; i < Nsnapshots; ++i){
    sprintf(f.in.name,"%s_%d.rst",file_name_in,fieldID[i]);
  ...
  //Substruct_mean ( master, nfields, Nsnapshots);



  compare_fields(master,0,4);
  compare_fields(master,1,5);
  compare_fields(master,2,6);
  ...
  ROOTONLY{
    ...
    for (j = 0; j < Nsnapshots; ++j){
      sprintf(dir_name,"VEL_DATA_%d",fieldID[j]);
...
//    Calc_Vort(&f,master,nfields, i);

  if(option("FEstorage")){
    for (i = 0; i < Nsnapshots; ++i){
      memset(file_name,'\0',BUFSIZ*sizeof(char));
      sprintf(file_name,"%s",file_name_out);
      WriteTEC(master, file_name, 4, fieldID[i], i );
    }
  }
...
void read_time_all_steps ( int Nsnapshots, double *Time_all_steps, int *fieldID,  char *file_name_in){
  ...
  ROOTONLY{
   ...
   memset(Time_all_steps,'\0',Nsnapshots*sizeof(double));
  }
  MPI_Bcast ( Time_all_steps, Nsnapshots, MPI_DOUBLE, 0, get_MPI_COMM());
    ...
    for(k = 0; k < Nel; ++k){
      ...
      if(Check_range_sub_cyl(F)){
        ...
        for(i = 0; i < ntot; ++i){
          ...
          for(n = 0; n < nfields;  ++n)
            XYZUVWP[index*(3+nfields)+3+n] = E[n+Snapshot_index]->flist[k]->h_3d[0][0][i];
          index ++;
        }
      }
    }
...
static void Calc_Write_WSS(Element_List **E, FileList f, Bndry *Ubc, int Snapshot_index){
  ...
  for(cnt=0,B = Ubc;B; B = B->next) {
    B->id = cnt++;
    if(B->type == 'W' || B->type == 'L') B->elmt->Surface_geofac(B);
  }
...
static void Calc_Write_WSS(Element_List *E, FileList f, Field fld, int Snapshot_index){
  ...
  /* calculate derivatives */
  for(cnt=0,B = Ubc;B; B = B->next)
    if((B->type == 'W' || B->type == 'L')&&(Check_surface(B,f)))
      for(i = 0; i < 3; ++i){
        F = B->elmt;
        eid = F->id;
...
/* sub ICA */
  ...
  if ( (zo > dparam("Zbox_max")) || (zo < dparam("Zbox_min")) )
    return 0.0;
...
void compare_fields(Element_List **E, int field_index,int field_index2 ){
  ...
  // compare fields from two  *rst files

  int i,j,k,n,qa,qb,qc;
  int qab, qbc, qt;
  double *z,*wa,*wb,*wc,*val,*tmp;
  double sum,Linf,L2,volume;
  Element *F, *F2;
  ...
  for(k = 0; k < E[0]->nel; ++k){
      ...
      F = E[field_index]->flist[k];
      F2 = E[field_index2]->flist[k];

      ROOTONLY
        if (k == 0){
    fprintf(stdout,"F: qa = %d qb = %d qc = %d \n", F->qa, F->qb, F->qc);
          fprintf(stdout,"F2: qa = %d qb = %d qc = %d \n", F2->qa, F2->qb, F2->qc);
        }
      ...
      memcpy(tmp,F2->h_3d[0][0],qt*sizeof(double));
      for (i = 0; i < qt; ++i)
  val[i] = abs(val[i]-tmp[i]);
  ...
  ROOTONLY
    fprintf(stdout,"Linf = %e ; L2 = %e ; volume = %f;  \n",Linf,L2, volume);
...
int reconstruct_field(double **V, Element_List **E, int field_index, int nfields, int Nmodes){
  ...
  int *mask;
  mask  = new int[nfields];

  memset(mask,'\0',nfields*sizeof(int));
  ...
  for (j = 0; j < nfields; ++j){
     if (mask[j] == 0)
       memset(V[j],'\0',nfields*sizeof(double));
  }
  delete[] mask;
