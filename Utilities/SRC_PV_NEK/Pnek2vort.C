...
main (int argc, char *argv[])
{
  ...
  fprintf(stderr,"setup is done \n");

  ReadCopyField(&f,master);

  fprintf(stderr,"ReadCopyField is done \n");
  gsync();


/*
  for (i = 1; i < Nsnapshots; ++i){
    sprintf(f.in.name,"RST_DATA/surf_test14_%d.rst",fieldID[i]);
...
*/
  ...
  fprintf(stderr,"before creating directories \n");
  ...
  if(option("FEstorage")){
    ...
    for (i = 0; i < Nsnapshots; ++i){
      ...
      //sprintf(file_name,"VORT_DATA_%d/%s_%d.dat",fieldID[i],strtok(f.rea.name,"."),mynode());
      //sprintf(file_name,"VORT_DATA_%d/%s_%d.dat",fieldID[i],f.out.name,mynode());
      sprintf(file_name,"VORT_DATA_%d/vort_%d.dat",fieldID[i],mynode());
      f.out.fp = fopen(file_name,"w");
      WriteS(master,f.out.fp, 4, i);
      fclose(f.out.fp);
    }
  }
