...
//  master  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
  master  = (Element_List **) malloc(12*sizeof(Element_List *));
  nfields = setup (&f, master, &nftot, 1,Ubc);
...
#if 1
    sprintf(file_name,"%s_%d.dat",strtok(f.rea.name,"."),mynode());
    f.out.fp = fopen(file_name,"w");
    Calc_Write_WSS(master, f, Ubc,0);
    fclose(f.out.fp);
