 ...
 *                        RCS Information                                    *
 *                                                                           *
 * $Source: /homedir/cvs/Nektar/Utilities/src/pressure.C,v $
 * $Revision: 1.2 $
...
static void Write(Element_List *E, FileList f, Field fld){
  ...
  qb = F->qb;
      ...
      // get pressure field
      F->Copy_field(fld.data[2]+data_skip[eid],fld.size+size_start);
      F->Trans(F,J_to_Q);
      F->GetFace (F->h[0],B->face,wk);
      ...
      fprintf(out,"ZONE T=\"Elmt %d Face %d\",I=%d, J=%d, F=POINT\n",
        F->id+1,B->face+1,qa,2);
      for(i = 0; i < qa; ++i)
  fprintf(out,"%lg %lg \n", X.x[i], X.y[i]);
      if(F->curvX)
  for(i = 0; i < qa; ++i)
    fprintf(out,"%lg %lg \n", X.x[i] + B->nx.p[i]*scal*p[i] ,
      X.y[i] + B->ny.p[i]*scal*p[i]);
      else
  for(i = 0; i < qa; ++i)
    fprintf(out,"%lg %lg \n", X.x[i] + B->nx.d*scal*p[i] ,
      X.y[i] + B->ny.d*scal*p[i]);
    }
