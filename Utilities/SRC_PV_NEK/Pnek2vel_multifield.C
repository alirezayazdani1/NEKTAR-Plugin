...
main (int argc, char *argv[])
{
  ...
  for (j = 0; j < Nsnapshots; ++j){
      ...
      if (j) {
        ...
        ReadAppendField(&f,master,nfields);
      }
...
//    Calc_Vort(&f,master,nfields, i);
  ...
  if(option("FEstorage")){
    ...
    for (i = 0; i < Nsnapshots; ++i){
      ...
      sprintf(file_name,"VEL_DATA_%d/surf_test14_%d.dat",fieldID[i],mynode());
      f.out.fp = fopen(file_name,"w");
...
static void WriteS(Element_List **E, FILE *out, int nfields, int Snapshot_index){
  ...
  qa = E[0+Snapshot_index]->flist[0]->qa;
