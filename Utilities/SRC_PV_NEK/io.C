...
{
  int  n;
  char buf[BUFSIZ], value[256], name[256], c;
  double val, dt;
  ...
  while (n--) {
    fgets (buf, BUFSIZ, fp);
    if(sscanf(buf, "%256s%256s", value, name) == 2) {
  ...
  if(strstr(buf,"type")){
	...
	if(!ntags) error_msg(No curved type information specified);
	  ...
	  if (fscanf(fp, "%lf%d%1s", &fac, &eid, &type) != 3){
            fprintf(stdout,"%lf%d%1s", fac, eid, type);
...
Bndry *ReadMeshBCs (FILE *fp, Element_List *Mesh){
  ...
  for(E=Mesh->fhead;E;E=E->next){ 
	...
	fprintf(stderr,"ReadBCs : %s\n", buf); exit(-1);
     ...
     /* Paris: Need this for ALE */
     if( ( bc == 'm'|| bc == 'L'  || bc == 'C' || bc == 'A') && !E->curvX ){
        E->curve       = (Curve*) calloc(1,sizeof(Curve));
        E->curve->type = T_Straight;
        E->curve->face = n;
        E->set_curved_elmt(Mesh);
     }
	...
	new_bndry->face = n;
	new_bndry->elmt = E;
      case 'W':  case 'V': case 'L': case 'G': case 'C': case 'A':
	E->set_solve(n,off);
	new_bndry = (Bndry *)calloc(1,sizeof(Bndry));
	new_bndry->type = toupper(bc);
	new_bndry->face = n;
	new_bndry->elmt = E;
	break;
      case 'M': case 't': case 'v': case 'm': case 's':
	E->set_solve(n,off);
	new_bndry = (Bndry *)calloc(1,sizeof(Bndry));
	new_bndry->type = 'V';
	new_bndry->face = n;
 ...
 /* ....... Begin Reading Boundary Conditions ...... */
  ...
  for(k =0; (k < pllinfo[active_handle].gnel) && (E); ++k){
	...
	case 'E': case 'P':
	  ...
	  break;
	default:
	  fprintf(stderr,"ReadBCs: read a strange boundary condition: %c\n", bc);
	  exit(-1);
	  break;
	}
	if (new_bndry) {
