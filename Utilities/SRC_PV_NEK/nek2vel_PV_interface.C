...
//  init_comm(&argc, &argv);
  ...
  for (j = 0; j < Nsnapshots; ++j){
     ...
     if(option("FEstorage")){
        sprintf(file_name,"VEL_DATA_%d/%s_%d.dat",fieldID[j],file_name_out,My_rank);
        //f.out.fp = fopen(file_name,"w");
        WriteVTK(master, file_name_out, 4, fieldID[j], FLAG_GEOM);
        FLAG_GEOM = 0;
        //fclose(f.out.fp);
     }
     else{
...
//	 iparam("MODES"), iparam("LQUAD"), iparam("MQUAD"), iparam("NQUAD"));
  ...
  DO_PARALLEL{ // recall global numbering to put partition vertices first
    ...
    //  fprintf(stderr, "nek2vel_PV_interface.C: mynode(%d) ** Time for LocalMesh()in DO_PARALLEL\n", mynode());
  ...
  //fprintf(stderr, "nek2vel_PV_interface.C: mynode(%d) (after DO_PARALLEL): %f\n", mynode(), timer_diff);
...
//    U[i+Snapshot_index]->Trans(U[i+Snapshot_index],J_to_Q);
  ...
  if(U[0]->fhead->dim() == 3){
    ...
    for(k = 0; k < U[0]->nel; ++k){
      ...
      U[uvw_index+0+Snapshot_index]->flist[k]->Grad_d(Ux, Uy, Uz, 'a');
      U[uvw_index+1+Snapshot_index]->flist[k]->Grad_d(Vx, Vy, Vz, 'a');
      U[uvw_index+2+Snapshot_index]->flist[k]->Grad_d(Wx, Wy, Wz, 'a');

      /*************** added code here ************************/
      /* compute lambda_2 at each quadrature point */
      for(i=0;i<qt;++i){
        S11=S12=S13=S22=S23=S33=T12=T13=T23=0.0;
        p11=p12=p13=p22=p23=p33=0.0;
        //U[3+Snapshot_index]->flist[k]->h_3d[0][0][i]=0.0;
	U[p_index+Snapshot_index]->flist[k]->h_3d[0][0][i]=0.0;
        ...
        //U[3+Snapshot_index] will store the lambda_2 (scalar)
        //dspev  is LAPACK function

        dspev('N','U',3,EigV,Lam,*Z,3,*WORK,INFO);
        if (INFO) error_msg("WARNING: Vortex Core Method -dspev- INFO is not zero\n");
        //U[3+Snapshot_index]->flist[k]->h_3d[0][0][i]=Lam[1];
        //U[3+Snapshot_index]->fhead->type = 'l';
	U[p_index+Snapshot_index]->flist[k]->h_3d[0][0][i]=Lam[1];
        U[p_index+Snapshot_index]->fhead->type = 'l';
      }
      /**************** added code ends here *******************/


      dvsub(qt, Wy, 1, Vz, 1, **U[uvw_index+0+Snapshot_index]->flist[k]->h_3d, 1);
      dvsub(qt, Uz, 1, Wx, 1, **U[uvw_index+1+Snapshot_index]->flist[k]->h_3d, 1);
      dvsub(qt, Vx, 1, Uy, 1, **U[uvw_index+2+Snapshot_index]->flist[k]->h_3d, 1);
	...
	//ROOTONLY
      ...
      Element_List *T  = U[uvw_index+0+Snapshot_index]->gen_aux_field('T');
      //ROOTONLY
      ...
      Bsys->lambda = (Metric*) calloc(U[uvw_index+0+Snapshot_index]->nel, sizeof(Metric));
      //ROOTONLY
      //  fprintf(stderr," %d ", U[uvw_index+0+Snapshot_index]->nel);
      ...
      dcopy(U[uvw_index+0+Snapshot_index]->htot, U[uvw_index+0+Snapshot_index]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      //ROOTONLY fprintf(stderr,"Projecting Wx\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[uvw_index+0+Snapshot_index], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[uvw_index+0+Snapshot_index]->base_hj, 1);
      U[uvw_index+0+Snapshot_index]->fhead->type = 'a';


      /* project Wy */

      dcopy(U[uvw_index+1+Snapshot_index]->htot, U[uvw_index+1+Snapshot_index]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      //ROOTONLY fprintf(stderr,"Projecting Wy\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[uvw_index+1+Snapshot_index], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[uvw_index+1+Snapshot_index]->base_hj, 1);
      U[uvw_index+1+Snapshot_index]->fhead->type = 'b';


     /* project Wz */

      dcopy(U[uvw_index+2+Snapshot_index]->htot, U[uvw_index+2+Snapshot_index]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      //ROOTONLY fprintf(stderr,"Projecting Wz\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[uvw_index+2+Snapshot_index], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[uvw_index+2+Snapshot_index]->base_hj, 1);
      U[uvw_index+2+Snapshot_index]->fhead->type = 'c';


      /* project lambda */

      dcopy(U[uvw_index+3+Snapshot_index]->htot, U[uvw_index+3+Snapshot_index]->base_h, 1, Tf->base_h, 1);
      Tf->Set_state('p');
      //ROOTONLY fprintf(stderr,"Projecting Lambda\n");
      solve(T, Tf, NULL, Bsys, Mass);
      T->Trans(U[uvw_index+3+Snapshot_index], J_to_Q);
      dcopy(T->hjtot, T->base_hj, 1, U[uvw_index+3+Snapshot_index]->base_hj, 1);
      U[uvw_index+3+Snapshot_index]->fhead->type = 'l';
     ...
     /********** PROJECTION DONE ******************/
      ...
      if(option("DUMPFILE")){
        ...
        DO_PARALLEL{
          ...
          sprintf(out1,"%s.fld.hdr.%d",out,pllinfo[get_active_handle()].procid);
          fp[0] = fopen(out1,"w");
          sprintf(out1,"%s.fld.dat.%d",out,pllinfo[get_active_handle()].procid);
          fp[1] = fopen(out1,"w");
        }
        else{
...
static void WriteS(Element_List **E, FILE *out, int nfields, int Snapshot_index){
  ...
  if(E[0+Snapshot_index]->fhead->dim() == 3){
    ...
    /* gather data from all pertiotions and dump it */
    int Nvert_global,Ncores,MyRank;
...
static void Calc_Write_WSS(Element_List *E, FileList f, Field fld, int Snapshot_index){
  ...
  for(cnt=0,B = Ubc;B; B = B->next) {
    B->id = cnt++;
    if(B->type == 'W' || B->type == 'L' || B->type == 's' || B->type == 'G') B->elmt->Surface_geofac(B);
  }
  ...
  /* calculate derivatives */
  for(cnt=0,B = Ubc;B; B = B->next)
    if((B->type == 'W' || B->type == 'L' || B->type == 's' || B->type == 'G')&&(Check_surface(B,f)))
      for(i = 0; i < 3; ++i){
        F = B->elmt;
        eid = F->id;
...
/* Updates the vertices of the deformed mesh 
 * Input: U, Element list of size 'nfields'
 * Ubc, boundary structure
 * iptr, location of the mesh deformation 
 *       variables in the master element list.
 * mode, 0 static->dynamic, 1 dynamic->static
 * Typical call from PV plugin to switch to dynamic mesh: Set_Mesh(this->master, this->Ubc, this->nfields, this->dynamic_coord_index, 0);
 * Typical call from PV plugin to switch to static mesh:  Set_Mesh(this->master, this->Ubc, this->nfields, this->dynamic_coord_index, 1);
 *   */
...
void Set_Mesh(Element_List **U, int nfields, int iptr, double scale_val, int mode){
  ...
  double XSCAL = scale_val; /* Scaling factors for dynamic mesh */
  double YSCAL = scale_val;
  double ZSCAL = scale_val;
  
  int source;

  if ( U[iptr]->fhead->type == 'i' && U[iptr+1]->fhead->type == 'j' && U[iptr+2]->fhead->type == 'k')
    source = 0; /* Fluid */
  if ( U[iptr]->fhead->type == 'u' && U[iptr+1]->fhead->type == 'v' && U[iptr+2]->fhead->type == 'w')
    source = 1; /* Solid */
  ...
  switch(source)
  {
    ...
    switch(mode)
    {
      case 0: /* Static to dynamic */
       {
       for(k = 0; k < U[0]->nel; ++k)
         {
         F = U[0]->flist[k];
         /* Get the coordinates for this element in the deformed continuum mesh */
#if 0

         ntot = Interp_symmpts(U[ iptr ]->flist[k], 2, U[ iptr ]->flist[k]->h_3d[0][0], X.x,'p');        
         ntot = Interp_symmpts(U[iptr+1]->flist[k], 2, U[iptr+1]->flist[k]->h_3d[0][0], X.y,'p');
         ntot = Interp_symmpts(U[iptr+2]->flist[k], 2, U[iptr+2]->flist[k]->h_3d[0][0], X.z,'p');

         for (i = 0; i < F->Nverts; i++)
           {
           X.x[i] = F->vert_store[i].x + XSCAL*(X.x[i] - F->vert_store[i].x);
           X.y[i] = F->vert_store[i].y + YSCAL*(X.y[i] - F->vert_store[i].y);
           X.z[i] = F->vert_store[i].z + ZSCAL*(X.z[i] - F->vert_store[i].z);
           }
#else
         for (i = 0; i < F->Nverts; i++)
           {
             X.x[i] = F->vert_store[i].x + XSCAL*(U[ iptr ]->flist[k]->vert[0].hj[i] - F->vert_store[i].x);
             X.y[i] = F->vert_store[i].y + YSCAL*(U[iptr+1]->flist[k]->vert[0].hj[i] - F->vert_store[i].y);
             X.z[i] = F->vert_store[i].z + ZSCAL*(U[iptr+2]->flist[k]->vert[0].hj[i] - F->vert_store[i].z);
           }
#endif
         ...
         if(F->curvX)
           ...
           for (i = 0; i < F->Nedges; i++)
             {
             for (j = 0; j < F->edge[i].l; ++j)
               {
               F->curvX[0].Cedge[i][j] = F->curvX[0].Cedge_store[i][j] + XSCAL*(U[ iptr ]->flist[k]->edge[i].hj[j] -  F->curvX[0].Cedge_store[i][j]);
               F->curvX[1].Cedge[i][j] = F->curvX[1].Cedge_store[i][j] + YSCAL*(U[iptr+1]->flist[k]->edge[i].hj[j] -  F->curvX[1].Cedge_store[i][j]);
               F->curvX[2].Cedge[i][j] = F->curvX[2].Cedge_store[i][j] + ZSCAL*(U[iptr+2]->flist[k]->edge[i].hj[j] -  F->curvX[2].Cedge_store[i][j]);
               }
             }
           ...
           for( i = 0; i < F->Nfaces; ++i)
             {
             ll = F->face[i].l;
             ll = (F->Nfverts(i) == 3) ? ll*(ll+1)/2 : ll*ll;
             if(ll)
              {
              for( j = 0; j < ll; ++j)
                {
                F->curvX[0].Cface[i][0][j] = F->curvX[0].Cface_store[i][0][j] + XSCAL*(U[ iptr ]->flist[k]->face[i].hj[0][j] - F->curvX[0].Cface_store[i][0][j]);
                F->curvX[1].Cface[i][0][j] = F->curvX[1].Cface_store[i][0][j] + YSCAL*(U[iptr+1]->flist[k]->face[i].hj[0][j] - F->curvX[0].Cface_store[i][0][j]);
                F->curvX[2].Cface[i][0][j] = F->curvX[2].Cface_store[i][0][j] + ZSCAL*(U[iptr+2]->flist[k]->face[i].hj[0][j] - F->curvX[0].Cface_store[i][0][j]);
                }
              }
             }
           }
         ...
         /* Move vertices and compute goemetric factors */
         U[0]->flist[k]->move_vertices(&X);
         U[0]->flist[k]->set_geofac();
         }
	 ...
	 for(n = 1; n < nfields; ++n)
           {
	   U[n]->flist[k]->move_vertices(&X); /* Functions from Hlib/src/Coords.C and Geofac.C */
           U[n]->flist[k]->geom = U[0]->flist[k]->geom;
           U[n]->flist[k]->curve = U[0]->flist[k]->curve;
           U[n]->flist[k]->curvX = U[0]->flist[k]->curvX;
           }
         }

       /* Update boundary */
       //for(Bc=Ubc;Bc;Bc=Bc->next)
         //U[0]->flist[Bc->elmt->id]->Surface_geofac(Bc);

      break;
      }
    case 1: /* Dynamic to static */
      {
        for(k = 0; k < U[0]->nel; ++k)
          {
          /* Get the coordinates for this element in the static continuum mesh */
          F  = U[0]->flist[k];  
          for (i = 0; i < F->Nverts; i++){
            X.x[i] = F->vert_store[i].x;
            X.y[i] = F->vert_store[i].y;
            X.z[i] = F->vert_store[i].z;
          }

          /* Move vertices and compute goemetric factors */
          U[0]->flist[k]->move_vertices(&X);
          U[0]->flist[k]->set_geofac();
          U[0]->flist[k]->set_curved_elmt(U[0]);
          }
      ...
      switch(mode)
      {
        case 0: /* Static to dynamic */
        {
         for(k = 0; k < U[0]->nel; ++k)
           ...
           for (i = 0; i < F->Nverts; i++)
             {
             X.x[i] = F->vert_store[i].x + XSCAL*U[ iptr ]->flist[k]->vert[0].hj[i];
             X.y[i] = F->vert_store[i].y + YSCAL*U[iptr+1]->flist[k]->vert[0].hj[i];
             X.z[i] = F->vert_store[i].z + ZSCAL*U[iptr+2]->flist[k]->vert[0].hj[i];
             }
#endif
           if(F->curvX)
             {
	     for (i = 0; i < F->Nedges; i++)
               ...
               for (j = 0; j < F->edge[i].l; ++j)
                 {
                 F->curvX[0].Cedge[i][j] = F->curvX[0].Cedge_store[i][j] + XSCAL*U[ iptr ]->flist[k]->edge[i].hj[j];
                 F->curvX[1].Cedge[i][j] = F->curvX[1].Cedge_store[i][j] + YSCAL*U[iptr+1]->flist[k]->edge[i].hj[j];
                 F->curvX[2].Cedge[i][j] = F->curvX[2].Cedge_store[i][j] + ZSCAL*U[iptr+2]->flist[k]->edge[i].hj[j];
                 }
               }
		...
		for( j = 0; j < ll; ++j)
                 {
                 F->curvX[0].Cface[i][0][j] = F->curvX[0].Cface_store[i][0][j] + XSCAL*U[ iptr ]->flist[k]->face[i].hj[0][j];
                 F->curvX[1].Cface[i][0][j] = F->curvX[1].Cface_store[i][0][j] + YSCAL*U[iptr+1]->flist[k]->face[i].hj[0][j];
                 F->curvX[2].Cface[i][0][j] = F->curvX[2].Cface_store[i][0][j] + ZSCAL*U[iptr+2]->flist[k]->face[i].hj[0][j];
                 }
               }
             }
#if 0
           ...
           if( ntot != F->Nverts ){
             fprintf(stdout,"Something's wrong in Set_Mesh!\n"); exit(-1);
           }


           for (i = 0; i < F->Nverts; i++)
             {
             X.x[i] = F->vert_store[i].x + XSCAL*X.x[i];
             X.y[i] = F->vert_store[i].y + YSCAL*X.y[i];
             X.z[i] = F->vert_store[i].z + ZSCAL*X.z[i];
             }
#endif
...
void Store_static_mesh(Element_List *U){
  ...
  for(k = 0; k < U->nel; ++k)
    {
    F  = U->flist[k];
    F->vert_store = (Vert *)calloc(F->Nverts,sizeof(Vert));


    for(i = 0; i < F->Nverts; ++i)
      {
      F->vert_store[i].x = F->vert[i].x;
      F->vert_store[i].y = F->vert[i].y;
      F->vert_store[i].z = F->vert[i].z;
      }
    ...
    if(F->curvX)
      ...
      for(i = 0; i < F->dim(); ++i)
        ...
        for(j = 0; j < F->Nedges; ++j)
          F->curvX[i].Cedge_store[j] = (double *)calloc(max(F->edge[j].l,1),sizeof(double));
        for(j = 0; j < F->Nfaces; ++j) 
          {
          F->curvX[i].Cface_store[j] = dmatrix(0, QGmax-1, 0, QGmax-1);
          dzero(QGmax*QGmax, F->curvX[i].Cface_store[j][0], 1);
          }
        }
      ...
      for (i = 0; i < F->Nedges; i++)
        ...
        for (j = 0; j < F->edge[i].l; ++j)
          {
          F->curvX[0].Cedge_store[i][j] = F->curvX[0].Cedge[i][j];
          F->curvX[1].Cedge_store[i][j] = F->curvX[1].Cedge[i][j];
          F->curvX[2].Cedge_store[i][j] = F->curvX[2].Cedge[i][j];
          }
        }
      ...
      for( i = 0; i < F->Nfaces; ++i)
        ...
        if(ll)
          ...
          for(j = 0; j < ll; ++j)
            {
            F->curvX[0].Cface_store[i][0][j] = F->curvX[0].Cface[i][0][j];
            F->curvX[1].Cface_store[i][0][j] = F->curvX[1].Cface[i][0][j];
            F->curvX[2].Cface_store[i][0][j] = F->curvX[2].Cface[i][0][j];
            }
          }
	}
      }
...
void Calc_StressTensor(Element_List **U, int displacement_index, int storage_index){
  ...
  htot = U[displacement_index+0]->htot;

  double **derivData = DMath::newD(9, htot);
  double *ux = derivData[0], *uy = derivData[1], *uz = derivData[2],
  *vx = derivData[3], *vy = derivData[4], *vz = derivData[5],
  *wx = derivData[6], *wy = derivData[7], *wz = derivData[8];

  U[displacement_index+0]->Grad_h(U[displacement_index+0]->base_h, ux, uy, uz, 'a'); // du/dx, du/dy, du/dz
  U[displacement_index+1]->Grad_h(U[displacement_index+1]->base_h, vx, vy, vz, 'a'); // dv/dx, dv/dy, dv/dz
  U[displacement_index+2]->Grad_h(U[displacement_index+2]->base_h, wx, wy, wz, 'a'); // dw/dx, dw/dy, dw/dz

  int moption = iparam("MATERIALOPTION");

  double sigma_x_min, sigma_x_max;
  double sigma_x, sigma_y, sigma_z, tau_xy, tau_yz, tau_zx;

  double **StrainE=DMath::newD(6,htot);
  memset(&StrainE[0][0], '\0', sizeof(double)*6*(htot));


  for(k = 0;k < htot; k++) {
    StrainE[0][k] = 0.5*((ux[k]+1.0)*(ux[k]+1.0) + vx[k]*vx[k] + wx[k]*wx[k] - 1.0);//E11
    StrainE[3][k] = ((ux[k]+1.0)*uy[k] + vx[k]*(vy[k]+1.0) + wx[k]*wy[k]);//E12
    StrainE[4][k] = ((ux[k]+1.0)*uz[k] + vx[k]*vz[k] + wx[k]*(wz[k]+1.0));//E13
    StrainE[1][k] = 0.5*(uy[k]*uy[k] + (vy[k]+1.0)*(vy[k]+1.0) + wy[k]*wy[k] - 1.0);//E22
    StrainE[5][k] = (uy[k]*uz[k] + (vy[k]+1.0)*vz[k] + wy[k]*(wz[k]+1.0));//E23
    StrainE[2][k] = 0.5*(uz[k]*uz[k] + vz[k]*vz[k] + (wz[k]+1.0)*(wz[k]+1.0) - 1.0);//E33
  }

  switch(moption){
    case 0:
      for( k = 0; k < htot; k++){
  	double ym = dparam("DMODULUS");
        double pr = dparam("DPOISSONRATIO");
        double E1 = ym*(1-pr)/((1+pr)*(1-2*pr));
        double E2 = ym*pr/((1+pr)*(1-2*pr));
        double E3 = 0.5*ym/(1+pr);

        sigma_x = E1*StrainE[0][k] + E2*StrainE[1][k] + E2*StrainE[2][k]; // sigma_x
        sigma_y = E2*StrainE[0][k] + E1*StrainE[1][k] + E2*StrainE[2][k]; // sigma_y
        sigma_z = E2*StrainE[0][k] + E2*StrainE[1][k] + E1*StrainE[2][k]; // sigma_z
        tau_xy  = E3*StrainE[3][k]; // tau_{xy}
        tau_zx  = E3*StrainE[4][k]; // tau_{xz}
        tau_yz  = E3*StrainE[5][k]; // tau_{yz}
	...
	if(sigma_x < sigma_x_min)
        ...
        U[displacement_index+0]->base_h[k] = sigma_x;
        U[displacement_index+1]->base_h[k] = sigma_y;
        U[displacement_index+2]->base_h[k] = sigma_z;
        U[storage_index+0]->base_h[k] = tau_xy;
        U[storage_index+1]->base_h[k] = tau_zx;
        U[storage_index+2]->base_h[k] = tau_yz;

    }
    break;
    case 2: case 3: 
      double *Energy1 = DMath::newD(2*U[displacement_index+0]->htot);
      double *Energy2 = &(Energy1[U[displacement_index+0]->htot]);
      double A10 = dparam("DMU10");
      double A01 = dparam("DMU01");
      double bm = dparam("DBULKMODULUS");
      int Nfiber = iparam("NFIBER");
      double alpha[4], I4[4], R_C[3][3], dum[3], c1[4], c2[4];
      double dedI4[4], dI4dE11[4], dI4dE12[4], dI4dE13[4], dI4dE22[4], dI4dE23[4], dI4dE33[4];
      double ***a0 = 0;
      double first_P[6], cauchy[6];

      if (Nfiber != 0){
        if(!a0) a0 = DMath::newD(U[displacement_index+0]->htot,Nfiber,3);
        memset(&a0[0][0][0], '\0', sizeof(double)*3*Nfiber*(U[displacement_index+0]->htot));
        if (Nfiber == 2){
          alpha[0] = dparam("DALPHA1");
          alpha[1] = dparam("DALPHA2");
          c1[0] = dparam("DC11");
          c2[0] = dparam("DC21");
          c1[1] = dparam("DC12");
          c2[1] = dparam("DC22");
          ...
          alpha[0] = dparam("DALPHA1");
          alpha[1] = dparam("DALPHA2");
          alpha[2] = dparam("DALPHA3");
          alpha[3] = dparam("DALPHA4");
          c1[0] = dparam("DC11");
          c2[0] = dparam("DC21");
          c1[1] = dparam("DC12");
          c2[1] = dparam("DC22");
        ...
        for (int i = 0; i < U[displacement_index+0]->htot; i++){
          for (int j = 0; j < Nfiber; j++){
            a0[i][j][0] = cos(M_PI*alpha[j]/180.);
            a0[i][j][1] = sin(M_PI*alpha[j]/180.);
            a0[i][j][2] = 0.0;
          }
        }
      }

      for(k = 0; k < U[displacement_index+0]->htot; k++){
        /* Reconstitui componentes do tensor C */
	double C11 = 2.*StrainE[0][k]+1., C12 = StrainE[3][k], C13 = StrainE[4][k],
	       C22 = 2.*StrainE[1][k]+1., C23 = StrainE[5][k], C33 = 2.*StrainE[2][k]+1.;

	double C12C12 = C12*C12, C13C13 = C13*C13, C23C23 = C23*C23;
	/* Calculo dos invariantes de C */
	/* I1 = tr(C) */
  	double I1 = C11 + C22 + C33;
  	/* I2 = 0.5*[tr(C)*tr(C) - tr(C*C)] */
	double I2 = 0.5*(I1*I1 - (C11*C11 + 2.*C12C12 + 2.*C13C13 + C22*C22 + 2.*C23C23 + C33*C33));
  	/* I3 = det(C) */
 	double I3 = C11*C22*C33 + 2.*C13*C12*C23 - C22*C13C13 - C12C12*C33 - C11*C23C23;
  	/* J = det(F) */
  	double J = sqrt(I3);

        double pbar = bm*(J-1.0);

  	/* Reconstitui os componentes do tensor E */
  	double E11 = StrainE[0][k], E12 = StrainE[3][k]/2., E13 = StrainE[4][k]/2.,
    	E22 = StrainE[1][k], E23 = StrainE[5][k]/2., E33 = StrainE[2][k];

  	/* Constantes auxiliares */
  	double Cte1 = 2.*A10/pow(J, 2./3.);
  	double Cte2 = 4.*A01/pow(J, 4./3.);
  	double Cte3 = -A10*I1/(3.*pow(J, 8./3.)) -A01*2.*I2/(3.*pow(J, 10./3.)) + pbar/(2.*J);

  	/* Componentes do tensor S (segundo tensor de Piola-Kirchhoff) */
      	Energy1[k] = A01*(pow(J,-4./3.)*I2-3.0)+A10*(pow(J,-2./3.)*I1-3.0);
       	Energy2[k] = bm/2.0*(J-1.)*(J-1.);

  	U[displacement_index+0]->base_h[k] = Cte1 + Cte2*(1. + E22 + E33)
    			    + Cte3*(4.*(.5 + E22 + E33) + 8.0*(E22*E33 - E23*E23)); //Sx  = S11

  	U[displacement_index+1]->base_h[k] = Cte1 + Cte2*(1. + E11 + E33)
    			    + Cte3*(4.*(.5 + E11 + E33) + 8.0*(E11*E33 - E13*E13)); //Sy  = S22

  	U[displacement_index+2]->base_h[k] = Cte1 + Cte2*(1. + E11 + E22)
    		    + Cte3*(4.*(.5 + E11 + E22) + 8.0*(E11*E22 - E12*E12)); //Sz  = S33

  	U[storage_index+0]->base_h[k] = Cte2*(-E12)
    			     + Cte3*(-4.*E12 + 8.0*(E23*E13 - E12*E33));       //Sxy = S12

  	U[storage_index+1]->base_h[k] = Cte2*(-E13)
    			     + Cte3*(-4.*E13 + 8.0*(E23*E12 - E22*E13));       //Sxz = S13

  	U[storage_index+2]->base_h[k] = Cte2*(-E23)
    			     + Cte3*(-4.*E23 + 8.0*(E12*E13 - E11*E23));       //Syz = S23
  	...
  	if (Nfiber != 0){
    	  ...
    	  for (int j = 0; j < Nfiber; j++){
      	    dgemv('N', 3, 3, 1.0, &R_C[0][0], 3, &a0[k][j][0], 1, 0.0, dum, 1);
            I4[j] = ddot(3, &a0[k][j][0], 1, dum, 1);

            dedI4[j] = c1[j]*exp(c2[j]*(I4[j] - 1.0)*(I4[j] - 1.0))*(I4[j] - 1.0)/2.0;

      	    dI4dE11[j] = 2.0 * a0[k][j][0] * a0[k][j][0];
      	    dI4dE12[j] = 4.0 * a0[k][j][0] * a0[k][j][1];
      	    dI4dE13[j] = 4.0 * a0[k][j][0] * a0[k][j][2];
      	    dI4dE22[j] = 2.0 * a0[k][j][1] * a0[k][j][1];
      	    dI4dE23[j] = 4.0 * a0[k][j][1] * a0[k][j][2];
      	    dI4dE33[j] = 2.0 * a0[k][j][2] * a0[k][j][2];

      	    U[displacement_index+0]->base_h[k] += dedI4[j] * dI4dE11[j];
      	    U[displacement_index+1]->base_h[k] += dedI4[j] * dI4dE22[j];
      	    U[displacement_index+2]->base_h[k] += dedI4[j] * dI4dE33[j];
      	    U[storage_index+0]->base_h[k] += dedI4[j] * dI4dE12[j];
      	    U[storage_index+1]->base_h[k] += dedI4[j] * dI4dE13[j];
      	    U[storage_index+2]->base_h[k] += dedI4[j] * dI4dE23[j];
    	  }
  	}

    	first_P[0] = (ux[k]+1.0)*U[displacement_index+0]->base_h[k] + uy[k]*U[storage_index+0]->base_h[k] + uz[k]*U[storage_index+1]->base_h[k];
    	first_P[3] = (ux[k]+1.0)*U[storage_index+0]->base_h[k] + uy[k]*U[displacement_index+1]->base_h[k] + uz[k]*U[storage_index+2]->base_h[k];
    	first_P[4] = (ux[k]+1.0)*U[storage_index+1]->base_h[k] + uy[k]*U[storage_index+2]->base_h[k] + uz[k]*U[displacement_index+2]->base_h[k];
    	first_P[1] = vx[k]*U[storage_index+0]->base_h[k] + (vy[k]+1.0)*U[displacement_index+1]->base_h[k] + vz[k]*U[storage_index+2]->base_h[k];
    	first_P[5] = vx[k]*U[storage_index+1]->base_h[k] + (vy[k]+1.0)*U[storage_index+2]->base_h[k] + vz[k]*U[displacement_index+2]->base_h[k];
    	first_P[2] = wx[k]*U[storage_index+1]->base_h[k] + wy[k]*U[storage_index+2]->base_h[k] + (wz[k]+1.0)*U[displacement_index+2]->base_h[k];

    	cauchy[0] = first_P[0]*(ux[k]+1.0) + first_P[3]*uy[k] + first_P[4]*uz[k];
    	cauchy[3] = first_P[0]*vx[k] + first_P[3]*(vy[k]+1.0) + first_P[4]*vz[k];
    	cauchy[4] = first_P[0]*wx[k] + first_P[3]*wy[k] + first_P[4]*(wz[k]+1.0);
    	cauchy[1] = first_P[3]*vx[k] + first_P[1]*(vy[k]+1.0) + first_P[5]*vz[k];
    	cauchy[5] = first_P[3]*wx[k] + first_P[1]*wy[k] + first_P[5]*(wz[k]+1.0);
    	cauchy[2] = first_P[4]*wx[k] + first_P[5]*wy[k] + first_P[2]*(wz[k]+1.0);

    	dscal(6, 1.0/J, cauchy, 1);

    	U[displacement_index+0]->base_h[k] = cauchy[0];
    	U[displacement_index+1]->base_h[k] = cauchy[1];
    	U[displacement_index+2]->base_h[k] = cauchy[2];
    	U[storage_index+0]->base_h[k] = cauchy[3];
    	U[storage_index+1]->base_h[k] = cauchy[4];
    	U[storage_index+2]->base_h[k] = cauchy[5];
...
void Calc_StressTensor(Element_List **U, int displacement_index, int storage_index){
  ...
  sigma_x_min = 99999;
  sigma_x_max = -sigma_x_max;
  for(k = 0; k < U[displacement_index+0]->nel; ++k)
  {
    qt = U[displacement_index+0]->flist[k]->qtot;

    U[displacement_index+0]->flist[k]->Grad_d(Ux, Uy, Uz, 'a');
    U[displacement_index+1]->flist[k]->Grad_d(Vx, Vy, Vz, 'a');
    U[displacement_index+2]->flist[k]->Grad_d(Wx, Wy, Wz, 'a');

    for(i = 0; i < qt; ++i)
      {
      sigma_x = E1*Ux[i] + E2*Vy[i] + E2*Wz[i];
      sigma_y = E2*Ux[i] + E1*Vy[i] + E2*Wz[i];
      sigma_z = E2*Ux[i] + E2*Vy[i] + E1*Wz[i];
      tau_xy  = E3*(Uy[i] + Vx[i]);
      tau_yz  = E3*(Vz[i] + Wy[i]);
      tau_zx  = E3*(Wx[i] + Uz[i]);
      ...
      U[displacement_index+0]->flist[k]->h_3d[0][0][i] = sigma_x;
      U[displacement_index+1]->flist[k]->h_3d[0][0][i] = sigma_y;
      U[displacement_index+2]->flist[k]->h_3d[0][0][i] = sigma_z;
      U[storage_index+0]->flist[k]->h_3d[0][0][i] = tau_xy;
      U[storage_index+1]->flist[k]->h_3d[0][0][i] = tau_yz;
      U[storage_index+2]->flist[k]->h_3d[0][0][i] = tau_zx;

     }
  }

  fprintf(stdout, "Calc_StressTensor: E1= %f\tE2= %f\tE3= %f\tsigma_x_min= %f\tsigma_x_max= %f\n",
	  E1, E2, E3, sigma_x_min, sigma_x_max);
  free_dmatrix(d,0,0);
