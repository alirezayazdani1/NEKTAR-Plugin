...
void Prism::set_curved_elmt(Element_List *U){
  ...
  for(cur=curve;cur;cur=cur->next){
    ...
    /* Paris: Add support to Recon type */
    if(cur->type==T_Recon){
      if(Nfverts(fac) == 4){
        error_msg(Recon curve type is not yet supported on quad faces);
      }
      else{
        for(i = 0; i < Nfverts(fac); ++i){
          vn = vnum(fac,i);
    ...
    switch(cur->type){
    case T_Recon:
      genRecon     (cur, v[0], v[1], v[2], x, y, z, q);
      goto      TransformFace;
    case T_Straight:
      break; /* do nothing since mode are all zero */
    case T_Cylinder:
...
void Element::move_vertices(Coord *X){
  ...
  for(i=0;i<Nverts;++i){
    ...
    if(dim()==3)
      vert[i].z = X->z[i];
    iparam_set("FAMILIES",iparam("FAMILIES")-1); /* Paris: Fix vorticity projection for dynamic mesh */
  }
}
#endif
