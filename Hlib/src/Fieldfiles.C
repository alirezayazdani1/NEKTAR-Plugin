  ...
  case 'b':
    {
      ...
      if (checkfmt (strchr(f->format,'-')+1)&&(!option("NoByteSwap"))){
        fprintf(stdout,"Warning: field file appears to be wrong format (expecting %s):"
         "Byte swapping\n", BTYPE);
        Bswap = 1;
      }
...
void ReadDataShuffled(FILE *fp, Field *f){
  ...
  for(i = 0; i < f->nel; ++i)
    ...
    fprintf(stderr, "error reading field %c, element %d, should have read %d items\n",
      f->type[n],i+1,len[i]);
    exit(-1);
        }
      ...
      for(n = 0; n < nfields; ++n)
      {
	  int br = fread (tmp,sizeof(double),(size_t)len[i],fp);
	  //if(fread (tmp,sizeof(double),(size_t)len[i],fp) != len[i]){
	  if ( br != len[i]){
            fprintf(stderr, "error reading field %c, element %d, should have read: %d items, but read: %d \n",
                               f->type[n],i+1,len[i], br);
            if (NULL==fp)
	      fprintf(stderr, "fp == NULL!!\n");
            else
	      fprintf(stderr, "fp != NULL, so something else is wrong\n");
              exit(-1);
        }
      }
    }
